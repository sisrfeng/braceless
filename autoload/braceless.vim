" I had it in my head that blocks needed to stop when they hit another pattern  match.
" They just need to stop at lower-indented lines.
" I could hard-code  the stop pattern,
" but I don't want to break the magic spell that's making  this work.
...
fun! s:get_block_end(start, pattern)
    let end = line('$')
...
fun! s:get_block_until_blank(start)
    let end = line('$')
    let start = min([end, a:start])
    let lastline = end
...
fun! s:build_pattern(line, pattern, ...)
...
fun! braceless#scan_head(pat, flag, ...) abort
...
fun! braceless#scan_tail(pat, head)
...
fun! braceless#validsearch(pattern, flags, ...) abort
    let saved = winsaveview()
    let found = [0, 0]
    let last_found = [0, 0]
...
fun! braceless#head_bounds(...) abort
...
fun! braceless#collection_bounds(...) abort
    let flags_extra = ''
    let stopline = 0
    if a:0 >= 1
        let flags_extra = a:1
...
fun! braceless#docstring_bounds()
    let saved = winsaveview()
...
fun! braceless#select_block(pattern, ...)
    let ignore_empty = 0
    if a:0 != 0
        let ignore_empty = a:1
    en
...
fun! braceless#define_pattern(filetype, patterns)
...
fun! braceless#get_block_lines(line, ...)
...
fun! braceless#get_parent_block_lines(...)
    let saved = winsaveview()
